...
|---------|------|-------------|----------------|
| API Gateway (Auth + BFF Proxy) | 8005 | `backend/main.py` | `backend/docker-compose.yml` |
| CRM Service (Supervisor) | 8006 | `backend/servers/crm_server.py` | `backend/docker-compose.yml` |
...
| Database API | 8020 | `database/api_server.py` | `database/docker-compose.yml` |
...
```
POST /api/marketing/chat/v2 (Gateway :8005)
 └── CRM Service (:8006)
      └── CRMMessageAgent               agents/crm_message_agent/crm_message_agent.py
           └── supervisor workflow       agents/crm_message_agent/workflow.py
                ├── supervisor_agent     RouteDecision → Command(goto=...)
                ├── search_agent         도구 기반 DB/Search 조회
                ├── recommend_product_agent  → A2A call (port 8001)
                ├── generate_message_agent   → A2A call (port 8002)
                └── data_registration_agent  → A2A call (port 8003)
```
...
| 파일 | 역할 |
|------|------|
| `backend/main.py` | API Gateway, JWT/API Key 인증, BFF 프록시 |
| `backend/servers/crm_server.py` | CRM Supervisor 서비스 (port 8006) |
| `backend/app/config/settings.py` | 환경변수 → `Settings` 싱글턴 |
| `backend/app/core/auth.py` | 인증 프로바이더 팩토리 (jwt / api_key) |
| `backend/app/core/llm_factory.py` | `get_llm(model, temperature)` |
| `backend/app/core/logging.py` | `AgentLogger`, `get_logger` |
| `backend/app/core/models.py` | API 계약용 Pydantic 모델 (User, Persona, Product 등) |
| `backend/app/core/rate_limiter.py` | PostgreSQL 기반 Rate Limiter |
| `backend/app/api/auth_router.py` | `/auth/*` 엔드포인트 (login, register, token refresh) |
| `backend/app/api/crm_proxy.py` | `/api/crm/*` → CRM 서비스 프록시 |
| `backend/app/api/db_proxy.py` | `/api/db/*` → Database API 프록시 |
| `backend/app/api/persona_pipeline.py` | `/api/personas/*` (업로드, 목록, 분석) |
| `backend/app/api/products_pipeline.py` | `/api/products/*` (업로드, 목록, 검색) |
| `backend/app/api/upload_jobs.py` | `/api/jobs/*` (백그라운드 작업 상태 SSE) |
| `backend/app/agents/base/base_state.py` | 모든 state의 기반 TypedDict |
| `backend/a2a/models.py` | A2A 프로토콜 공통 모델 |
| `backend/a2a/client.py` | A2A HTTP 클라이언트 (내부 인증 포함) |
| `backend/a2a/serialization.py` | A2A 호출용 state 직렬화 헬퍼 |
