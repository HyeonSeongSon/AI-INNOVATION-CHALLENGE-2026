...
│   └── logging.py                # structlog + RequestLoggingMiddleware (※ 신규 추가 파일)
├── routers/
...
├── requirements.txt
├── setup_pipeline.py             # 인덱스/파이프라인 초기 셋업
...
1. `cd opensearch && python setup_pipeline.py` — 인덱스/검색 파이프라인 셋업.
...
python setup_pipeline.py      # 인덱스/파이프라인 셋업
...
### 부록: 확인되지 않은/추가 확인 필요 항목
- 프론트엔드 Vite dev 포트(기본 5173) vs Docker 노출 포트(3000) 차이 — `vite.config.js`/`nginx.conf` 추가 확인 필요.
- `langgraph dev`용 `langgraph.json`(루트) 존재 여부는 본 분석 범위(4개 디렉토리)에 포함되지 않아 미확인.
- `database/core/logging.py`는 git 상태상 신규 추가 파일(untracked)로, 다른 모듈의 logging 구현과 일부 다를 수 있음.
