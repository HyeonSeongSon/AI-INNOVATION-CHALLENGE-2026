...
[![CRM 메시지 생성 시연 영상](https://img.youtube.com/vi/BhDjZy1cUeg/maxresdefault.jpg)](https://youtu.be/BhDjZy1cUeg)
...
이 기준선 위에서 임베딩 가중치와 top-k를 **10회 반복 실험**하되, **평균 점수가 아니라 상위 순위일수록 적합도가 높아지는 정렬 품질**(rank1–rank5 격차)을 기준으로 최적 조합을 선택했습니다.

| 조합 | top-5 평균 | top-3 평균 | rank1 | rank5 | **rank1–rank5 격차** |
|------|:---:|:---:|:---:|:---:|:---:|
| baseline | 3.620 | 3.783 | 4.000 | 3.373 | +0.627 |
| k=10 | 3.629 | 3.867 | 3.983 | 3.288 | +0.695 |
| k=15 | 3.629 | 3.861 | 4.000 | 3.271 | +0.729 |
| k=20 | 3.647 | 3.856 | 4.050 | 3.271 | +0.779 |
| k=20 · weight_v1 | 3.616 | 3.828 | 3.967 | 3.220 | +0.746 |
| k=20 · weight_v2 | 3.683 | 3.872 | 3.950 | 3.322 | +0.628 |
| k=20 · weight_v3 | 3.657 | 3.850 | 4.000 | 3.288 | +0.712 |
| k=20 · weight_v4 <sup>†</sup> | **3.806** | 4.033 | 4.183 | 3.542 | +0.641 |
| k=10 · weight_v4 <sup>†</sup> | 3.759 | 3.972 | 4.133 | 3.390 | +0.744 |
| **k=15 · weight_v4 (최종 채택)** | 3.780 | **4.039** | **4.150** | 3.356 | **+0.794** |

> **평균 1위는 채택 조합이 아닙니다.** `k=20 · weight_v4`가 top-5 평균 3.806으로 가장 높지만
> 정렬 품질은 +0.641에 그쳐, 평균만 봤다면 이 조합을 골랐을 것입니다. 최종 채택 조합은
> **상위 순위일수록 점수가 단조 상승**하는 정렬 품질에서 1위입니다 (`gpt-5-mini` judge, N=60).
>
> <sup>†</sup> 이 두 회차는 `top5_results`만 있고 `weight_adjustment_eval_results.jsonl`이
> 저장소에 없습니다(콘솔 로그 기준). 나머지 8회차는 `eval/[*]weight_adjustment_eval_results.jsonl`
> 에서 재계산해 검증했습니다.
