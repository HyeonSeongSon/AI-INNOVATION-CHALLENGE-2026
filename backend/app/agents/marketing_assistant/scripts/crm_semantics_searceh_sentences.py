...
def load_sentences(data_path: Path) -> list[dict]:
    ...
    logging.info(f"데이터 버전: {version} (최종 수정: {last_updated})")

    docs = []
    for category_key, category in data["categories"].items():
        sentences = category.get("sentences", [])
        ...
        logging.info(
            f"  [{category['severity'].upper():8s}] {category_key}: {len(sentences)}개 문장"
        )

    logging.info(f"'{data_path.name}'에서 총 {len(docs)}개 문장을 로드했습니다.")
    return docs
