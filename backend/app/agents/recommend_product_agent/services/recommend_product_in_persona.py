...
class ProductRecommender:
    ...
    async def filtered_products(
        ...
        """브랜드·카테고리 조건으로 상품을 필터링하고 product_id 목록을 반환한다.

        결과 수가 `settings.min_filtered_products` 미만이면 조건을 단계적으로 완화한다.
        - 레벨 1: brands + sub_tags
        - 레벨 2: brands 제거 (sub_tags만)
        - 레벨 3: sub_tags도 제거 (전체 상품)
        - 레벨 4: 필터 전체 제거 (전체 상품)

        Args:
            brands: 브랜드 필터 문자열 (없으면 None)
            sub_tags: 카테고리 태그 목록 (없으면 None)

        Returns:
            필터링된 product_id 문자열 리스트
        """
        # 레벨 1: brands + sub_tags
        filtered_products = await self.product_client.get_filtered_products(
    ...
    async def recommend(
        ...
        # 상품 상세 정보 병렬 조회
        products = await self.product_client.get_products_detail_from_db(top_ids)
