...
def get_persona_recommendations(
    ...
    product_personas 테이블에서 매칭된 상품 반환
    """
    # 페르소나 확인
    persona = db.query(Persona).filter(
        Persona.persona_key == persona_key
    ...
    # 매칭된 상품 조회 (연관도 높은 순)
    recommendations = db.query(ProductPersona).options(
        joinedload(ProductPersona.product).joinedload(Product.brand)
    ).filter(
        ProductPersona.persona_id == persona.id
    ).order_by(
        ProductPersona.relevance_score.desc()
    ).limit(limit).all()
        ...
        "recommendations": [
            {
                ...
                "brand_name": rec.product.brand.name if rec.product.brand else None,
                "category": rec.product.category,
                "sale_price": float(rec.product.sale_price) if rec.product.sale_price else None,
                "rating": float(rec.product.rating) if rec.product.rating else None,
                "relevance_score": float(rec.relevance_score) if rec.relevance_score else None,
                "matched_attributes": rec.matched_attributes,
                "image_urls": rec.product.image_urls[:3] if rec.product.image_urls else []
            }
