...
logger = logging.getLogger("persona_analysis")
...
async def run_persona_analysis(persona_data: Dict[str, Any], model: Optional[str] = None) -> Dict[str, Any]:
    ...
    logger.info("persona_analysis_started | persona_name=%s model=%s", persona_name, model_name)
    ...
    try:
        ...
        logger.error("persona_analysis_llm_failed | persona_name=%s model=%s duration_ms=%s error=%s",
    ...
    try:
        ...
        duration_ms = round((time.perf_counter() - start) * 1000, 1)
        logger.warning("persona_analysis_json_parse_failed | persona_name=%s duration_ms=%s error=%s",
                       persona_name, duration_ms, str(e))
        result = {
            "multi_level_analysis": {},
            "multi_dimensional_analysis": {},
        }
    ...
    duration_ms = round((time.perf_counter() - start) * 1000, 1)
    logger.info("persona_analysis_completed | persona_name=%s model=%s duration_ms=%s",
