...
def _build_persona_description(data: Dict[str, Any]) -> str:
    lines = [
        ...
        f"고민: {', '.join(data.get('concerns', []))}",
        f"헤어 타입: {', '.join(data.get('hair_type', []))}",
        ...
        f"가치관: {', '.join(data.get('lifestyle_values', []))}",
        f"관심 뷰티 카테고리: {', '.join(data.get('beauty_interests', []))}",
        f"스킨케어 루틴: {', '.join(data.get('skincare_routine', []) if isinstance(data.get('skincare_routine'), list) else ([data.get('skincare_routine')] if data.get('skincare_routine') else []))}",
        f"주 활동 환경: {', '.join(data.get('main_environment', []) if isinstance(data.get('main_environment'), list) else ([data.get('main_environment')] if data.get('main_environment') else []))}",
        ...
        f"반려동물: {', '.join(data.get('pets', []) if isinstance(data.get('pets'), list) else ([data.get('pets')] if data.get('pets') else []))}",
        f"평균 수면: {data.get('avg_sleep_hours', '')}시간, 스트레스: {data.get('stress_level', '')}",
        f"스크린 사용: {data.get('daily_screen_hours', '')}시간/일",
        f"쇼핑 스타일: {', '.join(data.get('shopping_style', []) if isinstance(data.get('shopping_style'), list) else ([data.get('shopping_style')] if data.get('shopping_style') else []))}",
        f"가격 민감도: {data.get('price_sensitivity', '')}",
        f"선호 브랜드: {', '.join(data.get('preferred_brands', []))}",
        f"기피 브랜드: {', '.join(data.get('avoided_brands', []))}",
...
async def generate_persona_summary(persona_data: Dict[str, Any], model: str = None) -> str:
    """
    LLM으로 페르소나 요약 생성
    ...
    duration_ms = round((time.perf_counter() - start) * 1000, 1)
    logger.info("persona_summary_completed | persona_name=%s model=%s duration_ms=%s",
                persona_name, model_name, duration_ms)
