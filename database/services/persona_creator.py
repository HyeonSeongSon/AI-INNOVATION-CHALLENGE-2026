...
[skin_type / concerns — 키워드 전용 필드]
skin_type과 concerns는 반드시 표준 키워드 하나씩만 배열에 넣습니다. 서술형 문장 절대 금지.
- skin_type 변환 방법: 사용자 입력에서 피부 타입 특성을 파악해 표준 단어로 변환합니다.
  - "샤워 후 바로 당기고 하얗게 일어남" → "건성"
  - "피지 분비 많음, 번들거림" → "지성"
  - "T존 번들, 볼은 건조" → "복합성"
  - "자극에 예민, 발적" → "민감성"
  - 표준 단어 목록: 건성, 지성, 복합성, 민감성, 중성, 지복합성, 건복합성
- concerns 변환 방법: 사용자 입력에서 고민 증상을 파악해 표준 키워드로 변환합니다.
  - "컨디션에 따라 트러블 올라옴" → "트러블"
  - "피부가 푸석함" → "건조"
  - "잡티 있음" → "잡티"
  - 예시 키워드: 잡티, 붉은기, 건조, 트러블, 모공, 여드름, 피지, 탈모, 두피 지성, 주름, 탄력, 색소침착
...
_SEARCH_QUERY_SYSTEM_PROMPT = """사용자 입력에서 직접 언급한 내용만 사용해서 검색/매칭용 4개 쿼리(need / preference / retrieval / persona)를 생성하세요.
