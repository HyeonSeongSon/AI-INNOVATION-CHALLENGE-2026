"""
hair 특정 상품 페르소나 재생성 스크립트

지정된 product_id에 해당하는 eval 레코드의 페르소나를 재생성하여
synthetic_eval_dataset.jsonl에 덮어씁니다.

사용법:
    python eval/regenerate_hair_personas.py
    python eval/regenerate_hair_personas.py --concurrency 3
    python eval/regenerate_hair_personas.py --model gpt-5-mini
"""
import asyncio
import json
import argparse
...
def load_hair_products() -> list[dict]:
    """hair 데이터 파일에서 대상 상품만 로드"""
    path = _DATA_DIR / "v2_product_data_structured_hair.jsonl"
    products = []
