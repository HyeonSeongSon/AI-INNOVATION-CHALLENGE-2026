"""
페르소나 → 검색 쿼리 사전 생성 스크립트

eval_dataset의 각 레코드(persona_info)로부터 검색 쿼리를 미리 생성하여
eval_query_dataset.jsonl에 저장합니다.
run_eval.py 실행 시마다 LLM을 호출하는 비용/시간을 절약할 수 있습니다.

사용법:
    python eval/v4/v4_generate_eval_dataset_query.py
    python eval/v4/v4_generate_eval_dataset_query.py --input eval/v4_synthetic_eval_dataset.jsonl
    python eval/v4/v4_generate_eval_dataset_query.py --concurrency 5
"""
import asyncio
import json
import argparse
...
async def main(input_path: Path, output_path: Path, concurrency: int):
    ...
    targets = [r for r in all_records if r["eval_id"] not in existing_ids]
    total = len(targets)

    if total == 0:
        print("모든 레코드가 이미 생성됨. 종료.")
        return
...
def parse_args():
    parser = argparse.ArgumentParser(description="페르소나 → 검색 쿼리 데이터셋 생성")
    parser.add_argument(
