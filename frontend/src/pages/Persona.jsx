    ...
    const fetchPersonas = async () => {
      try {
        ...
        const formatted = rawData.map(p => ({
            ...
            petInfo: (p.pets && !(Array.isArray(p.pets) && p.pets.length === 0)) ? p.pets : '없음',
  ...
  const handleCreateFromText = async () => {
    ...
    try {
      ...
      const newPersona = {
        ...
        petInfo: (structured_persona.pets && !(Array.isArray(structured_persona.pets) && structured_persona.pets.length === 0)) ? structured_persona.pets : '없음',
