...
[스킨케어] → 피부 고민 중심
  ...
  - 페르소나가 자연어 텍스트인 경우 루틴·생활 묘사에서 사용 시점·행동을 추출하여 반영
    예: "저녁 세안 첫 단계로 손에 덜어 마사지하듯 녹여낸 뒤" → 1차 세안(오일/밤 타입) 신호
        "가방에서 꺼내 화장 위에 바로 덧바른다" → 낮 중간 즉각 보습(미스트 타입) 신호
        "세안 직후 가장 먼저 두드려 흡수" → 첫 케어 단계(토너/스킨 타입) 신호
...
3. retrieval_query
   - user_need_query + user_preference_query 핵심 + 사용 맥락(어떤 상황·순간에 쓰는지)을 결합
   - 단순 이어붙이기 금지, 자연스러운 단일 문장으로 작성
   - 페르소나 루틴·생활 묘사에서 "언제·어떻게 쓰는 제품인지" 를 반드시 1어절 이상 포함하세요
     예: ❌ "냉난방으로 인한 오후 당김과 화장 들뜸을 예방하는 저자극 고보습 제품"
         ✅ "냉난방으로 오후 볼이 당기고 화장이 들뜰 때 가방에서 꺼내 덧바르는 즉각 보습 제품"
     예: ❌ "메이크업·선크림 잔여물 제거와 세안 후 건조함 방지를 원하는 저자극 클렌저"
         ✅ "저녁 세안 첫 단계로 메이크업을 마사지하며 녹여낸 뒤 헹궈내는 저자극 클렌저"
