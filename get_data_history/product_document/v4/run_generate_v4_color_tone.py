"""
[v4] 색조 멀티벡터 문서 생성 스크립트

입력:  data/v3_product_data_rewritten_color_tone.jsonl
        → structured 필드에서 _original_semantic 제외한 값을 프롬프트 인풋으로 사용
출력:  get_data_history/product_document/v4/v4_product_data_color_tone.jsonl

각 상품당 생성 필드:
  combined      : 5개 텍스트
  function_desc : 4개 텍스트
  attribute_desc: 5개 텍스트
  target_user   : 3개 텍스트
  spec_feature  : 2개 텍스트

사용법:
  python run_generate_v4_color_tone.py

주의:
  - 이미 처리된 product_id는 건너뜁니다 (재시작 안전).
  - LLM 응답 검증 실패 시 1회 재시도 후 건너뜁니다.
  - 실패한 product_id는 failed_v4_color_tone.jsonl에 기록됩니다.
"""
...
from get_data_history.product_document.v4.prompts_group_b import build_group_b_prompt
...
INPUT_FILE  = DATA_DIR / "v3_product_data_rewritten_color_tone.jsonl"
OUTPUT_FILE = OUTPUT_DIR / "v4_product_data_color_tone.jsonl"
FAILED_FILE = OUTPUT_DIR / "failed_v4_color_tone.jsonl"
CONCURRENCY = 10
...
async def generate_product(
    ...
    async with semaphore:
        ...
        prompt = build_group_b_prompt(input_doc)
