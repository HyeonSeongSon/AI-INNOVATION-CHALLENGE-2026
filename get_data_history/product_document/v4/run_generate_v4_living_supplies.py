"""
[v4] 생활용품 멀티벡터 문서 생성 스크립트

입력:  data/v3_product_data_rewritten_living_supplies.jsonl
        → structured 필드에서 _original_semantic 제외한 값을 프롬프트 인풋으로 사용
출력:  get_data_history/product_document/v4/v4_product_data_living_supplies.jsonl
...
사용법:
  python run_generate_v4_living_supplies.py

주의:
  - 이미 처리된 product_id는 건너뜁니다 (재시작 안전).
  - LLM 응답 검증 실패 시 1회 재시도 후 건너뜁니다.
  - 실패한 product_id는 failed_v4_living_supplies.jsonl에 기록됩니다.
"""
...
INPUT_FILE  = DATA_DIR / "v3_product_data_rewritten_living_supplies.jsonl"
OUTPUT_FILE = OUTPUT_DIR / "v4_product_data_living_supplies.jsonl"
FAILED_FILE = OUTPUT_DIR / "failed_v4_living_supplies.jsonl"
CONCURRENCY = 10
...
def resolve_group(product: dict) -> str:
    ...
    if tag == "용기&수저":
        return "F"
    print(f"[WARN] 미매핑 태그 '{tag}' → 그룹F로 처리")
    return "F"
