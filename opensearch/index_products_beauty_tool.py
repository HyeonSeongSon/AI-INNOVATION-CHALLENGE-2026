...
STRUCTURED_KEYWORD_FIELDS = [
    ...
    # 뷰티툴 전용
    'device_type', 'technology', 'skin_function', 'usage_frequency', 'session_time',
    'power_source', 'tool_type', 'tool_function', 'accessory_type',
    'brush_area', 'bristle_type', 'bristle_feel', 'handle_material',
    'material', 'lash_style', 'band_type', 'capacity',
]
STRUCTURED_INT_FIELDS = [
    'intensity_levels',
]
STRUCTURED_BOOL_FIELDS = [
    'waterproof', 'medical_certified', 'washable', 'reusable', 'latex_free', 'airtight',
]
# dict/list-of-dict 구조 — 문자열로 변환해서 저장
STRUCTURED_OBJECT_FIELDS = [
    'included_items', 'hair_tool_spec',
]


def get_beauty_tool_new_field_mappings():
    """기존 인덱스에 없는 뷰티툴 전용 필드 매핑"""
    keyword_fields = [
        'device_type', 'technology', 'skin_function', 'usage_frequency', 'session_time',
        'power_source', 'tool_type', 'tool_function', 'accessory_type',
        'brush_area', 'bristle_type', 'bristle_feel', 'handle_material',
    ...
    properties["intensity_levels"] = {"type": "integer"}
    properties["waterproof"] = {"type": "boolean"}
    properties["medical_certified"] = {"type": "boolean"}
    properties["washable"] = {"type": "boolean"}
    properties["reusable"] = {"type": "boolean"}
    properties["latex_free"] = {"type": "boolean"}
    properties["airtight"] = {"type": "boolean"}
    properties["hair_tool_spec"] = {"type": "text"}
    return properties


def update_index_mapping(client, index_name: str):
    """기존 인덱스에 뷰티툴 전용 필드 매핑 추가 (이미 존재하는 필드는 건너뜀)"""
    new_properties = get_beauty_tool_new_field_mappings()
...
def load_and_prepare_documents(jsonl_file_path):
    """
    beauty_tool JSONL에서 문서를 로드하고 색인할 필드만 추출합니다.
        ...
        with open(jsonl_file_path, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                ...
                try:
                    ...
                    # object 필드는 JSON 문자열로 변환
                    for field in STRUCTURED_OBJECT_FIELDS:
                        val = structured.get(field)
                        if val is not None:
...
def index_beauty_tool_to_opensearch(
    jsonl_file_path,
    ...
    logging.info(f"'{index_name}' 인덱스에 뷰티툴 전용 필드 매핑 추가 중...")
...
if __name__ == "__main__":
    JSONL_FILE = get_absolute_path("data", "v2_product_data_structured_beauty_tool.jsonl")
    INDEX_NAME = "product_index_v3"
    RECREATE_INDEX = False  # 기존 인덱스에 추가

    print("=" * 60)
    print("뷰티툴 상품 데이터 OpenSearch 색인 시작")
    print("=" * 60)
    ...
    success = index_beauty_tool_to_opensearch(
