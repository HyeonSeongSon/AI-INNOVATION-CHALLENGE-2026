...
def get_hair_new_field_mappings():
    """기존 인덱스에 없는 헤어 전용 필드 매핑"""
    keyword_fields = [
        'scalp_type', 'hair_type', 'damage_level', 'treatment_type',
        'treatment_focus', 'hair_function', 'greasy_feel', 'application_timing',
    ]
    properties = {field: {"type": "keyword"} for field in keyword_fields}
    properties["leave_on_time"] = {"type": "text"}
    properties["fragrance"] = {"type": "text"}
    properties["heat_protection"] = {"type": "boolean"}
    return properties


def update_index_mapping(client, index_name: str):
    """기존 인덱스에 헤어 전용 필드 매핑 추가"""
    new_properties = get_hair_new_field_mappings()
    try:
...
def load_and_prepare_documents(jsonl_file_path):
    """
    hair JSONL에서 문서를 로드하고 색인할 필드만 추출합니다.
        ...
        with open(jsonl_file_path, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                ...
                try:
                    ...
                    for field in STRUCTURED_HAIR_TEXT_FIELDS:
                        val = structured.get(field)
                        if val is not None:
                            filtered_doc[field] = val
...
def index_hair_to_opensearch(
    ...
    logging.info(f"'{index_name}' 인덱스에 헤어 전용 필드 매핑 추가 중...")
...
if __name__ == "__main__":
    JSONL_FILE = get_absolute_path("data", "product_data_structured_hair.jsonl")
    INDEX_NAME = "product_index_v2"
    RECREATE_INDEX = False  # 기존 인덱스에 추가

    print("=" * 60)
    print("헤어 상품 데이터 OpenSearch 색인 시작")
    print("=" * 60)
    ...
    success = index_hair_to_opensearch(
