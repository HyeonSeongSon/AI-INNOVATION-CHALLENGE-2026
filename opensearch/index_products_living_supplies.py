...
STRUCTURED_TEXT_FIELDS = [
    ...
    'product_story', 'craftsmanship',
    # 생활도구 전용 텍스트 필드
    'capacity', 'noise_level', 'runtime', 'search_phrases',
]
STRUCTURED_KEYWORD_FIELDS = [
    'suitable_for', 'body_area', 'highlight_keywords',
    'tableware_type', 'material', 'tool_function', 'included_items',
    # 생활도구 전용 키워드 필드
    'appliance_type', 'power_source', 'technology',
...
def get_living_supplies_new_field_mappings():
    """기존 인덱스에 없는 생활도구 전용 필드 매핑"""
    properties = {
        # 생활도구 전용 텍스트 필드
        "craftsmanship": {"type": "text"},
        "capacity":      {"type": "text"},
        "noise_level":   {"type": "text"},
        "runtime":       {"type": "text"},
        # 생활도구 전용 키워드 필드
        "tableware_type": {"type": "keyword"},
        "material":       {"type": "keyword"},
        "tool_function":  {"type": "keyword"},
        "included_items": {"type": "keyword"},
...
def update_index_mapping(client, index_name: str):
    """기존 인덱스에 생활도구 전용 필드 매핑 추가 (이미 존재하는 필드는 건너뜀)"""
    new_properties = get_living_supplies_new_field_mappings()

    # 이미 존재하는 필드는 제외 (타입 충돌 방지)
    try:
...
def index_living_supplies_to_opensearch(
    jsonl_file_path,
    index_name="product_index_v3",
):
    """
    생활도구 상품 데이터를 기존 OpenSearch 인덱스에 색인합니다.

    Args:
        jsonl_file_path: JSONL 파일 경로
        index_name: 색인할 기존 인덱스 이름
    """
    ...
    if not index_exists:
        logging.error(f"'{index_name}' 인덱스가 존재하지 않습니다. 먼저 기본 인덱스를 생성하세요.")
        return False

    logging.info(f"'{index_name}' 인덱스에 생활도구 전용 필드 매핑 추가 중...")
...
if __name__ == "__main__":
    JSONL_FILE = get_absolute_path("data", "v3_product_data_rewritten_living_supplies.jsonl")
    INDEX_NAME = "product_index_v3"

    print("=" * 60)
    print("생활도구 상품 데이터 OpenSearch 색인 시작")
    print("=" * 60)
    print(f"JSONL 파일: {JSONL_FILE}")
    print(f"인덱스 이름: {INDEX_NAME}")
    print("=" * 60)

    success = index_living_supplies_to_opensearch(
